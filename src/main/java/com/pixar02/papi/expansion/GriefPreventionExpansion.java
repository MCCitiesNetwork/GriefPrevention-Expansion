...
public class GriefPreventionExpansion extends PlaceholderExpansion implements Configurable {
    ...
    public Map<String, Object> getDefaults() {
        ...
        defaults.put("translate.offline", "Player is offline");
        return defaults;
    }
    ...
    public String onRequest(OfflinePlayer p, String identifier) {
        ...
        if (identifier.equals("muted")){
            if (DataS.isSoftMuted(p.getUniqueId())){
        ...
        */
        ...
        if (identifier.equals("claimedblocks_current")) {
            if (!p.isOnline()) {
                return ChatColor.translateAlternateColorCodes('&',
                        getString("translate.offline", "Player is offline"));
            }
            Claim claim = DataS.getClaimAt(p.getPlayer().getLocation(), true, null);
            if (claim == null) {
                return ChatColor.translateAlternateColorCodes('&',
        ...
        */
        ...
        if (identifier.equals("currentclaim_ownername")) {
            ...
            if (!p.isOnline()) {
                return ChatColor.translateAlternateColorCodes('&',
                        getString("color.neutral", "") + getString("translate.offline", "Player is offline"));
            }
            Claim claim = DataS.getClaimAt(p.getPlayer().getLocation(), true, null);
            if (claim == null) {
                return ChatColor.translateAlternateColorCodes('&',
                ...
                boolean isTrusted = claim.allowAccess(p.getPlayer()) == null;
                if (isTrusted){
                    //Trusted
                    return ChatColor.translateAlternateColorCodes('&',
